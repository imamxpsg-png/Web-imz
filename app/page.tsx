...
export default function Home() {
  ...
  return (
    <main className="w-screen min-h-[200vh] bg-black text-white relative overflow-x-hidden">
      
      {/* ==================================================== */}
      {/* SLIDE 1: INTRO (TEKS BERJALAN & KREDENSIAL LENGKAP) */}
      {/* ==================================================== */}
      <section className="w-full h-screen flex flex-col justify-between items-center py-8 relative bg-black overflow-hidden z-20 shadow-[0_20px_50px_rgba(0,0,0,1)]">
        ...
        <header className="relative z-10 w-full max-w-7xl mx-auto flex items-center justify-between px-6">
          <div className="text-3xl font-black tracking-widest flex items-center gap-2 select-none">
          ...
          <div className="text-xs font-mono text-zinc-300 bg-black/40 px-3 py-1 rounded-full border border-zinc-700/40">[ INTRO ]</div>
        </header>

        <div className="relative z-10 w-full my-auto overflow-hidden whitespace-nowrap flex items-center">
          <div className="animate-text-marquee inline-block whitespace-nowrap">
            {["welcome to imz", "welcome to imz", "welcome to imz"].map((text, idx) => (
              <h1 key={idx} className="text-6xl md:text-9xl font-black uppercase tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-pink-500 to-amber-400 drop-shadow-[0_10px_40px_rgba(236,72,153,0.5)] select-none inline-block mx-4">{text}</h1>
            ))}
          </div>
        </div>

        <div className="relative z-10 flex flex-col items-center gap-1.5 mb-6 select-none opacity-80 animate-bounce">
          <span className="text-[10px] font-mono font-bold tracking-[0.25em] text-zinc-400 uppercase">Gulir Ke Bawah</span>
          <ArrowDown className="w-4 h-4 text-purple-400" />
        </div>

        <footer className="relative z-10 w-full max-w-7xl mx-auto px-6 border-t border-zinc-800/40 pt-6 mt-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 items-center text-center text-xs font-bold text-zinc-200">
      ...
      {/* ==================================================== */}
      <section className="w-full h-screen flex flex-col justify-between py-6 relative bg-zinc-950 overflow-hidden z-10">
        <div className="absolute inset-0 z-0 h-full w-full pointer-events-none">
        ...
        <header className="relative z-30 w-full max-w-7xl mx-auto px-4 md:px-6">
          <div className="flex items-center justify-between border border-zinc-800/80 bg-black/85 backdrop-blur-xl px-6 py-4 rounded-2xl w-full shadow-2xl relative text-xs font-semibold text-zinc-300">
            <span className="text-sm font-black text-purple-400 font-mono">IMZ MULTIMEDIA</span>
            <div className="flex gap-2">
              <Link href="/profil" passHref className="flex items-center gap-1 bg-zinc-900/60 px-2.5 py-1.5 rounded-lg border border-zinc-800/40 hover:text-white transition-all"><User size={13} className="text-blue-400" /> Profil</Link>
              <Link href="/project" passHref className="flex items-center gap-1 bg-zinc-900/60 px-2.5 py-1.5 rounded-lg border border-zinc-800/40 hover:text-white transition-all"><LayoutGrid size={13} className="text-purple-400" /> Project</Link>
              <Link href="/kerjasama" passHref className="flex items-center gap-1 bg-zinc-900/60 px-2.5 py-1.5 rounded-lg border border-zinc-800/40 hover:text-white transition-all"><Handshake size={13} className="text-amber-400" /> Kerjasama</Link>
              <Link href="/contact" passHref className="flex items-center gap-1 bg-zinc-900/60 px-2.5 py-1.5 rounded-lg border border-zinc-800/40 hover:text-white transition-all"><Mail size={13} className="text-pink-400" /> Contact</Link>
              <Link href="/cs" passHref className="flex items-center gap-1 bg-zinc-900/60 px-2.5 py-1.5 rounded-lg border border-zinc-800/40 hover:text-white transition-all"><Headphones size={13} className="text-emerald-400" /> CS</Link>
            </div>
          </div>
        </header>

        {/* AREA PANEL INTERAKTIF DIVISI TEKNOLOGI */}
        <div className="relative z-30 w-full max-w-7xl mx-auto px-4 md:px-6 mt-4 mb-auto grid grid-cols-1 md:grid-cols-12 gap-4">
          <div className="md:col-span-4 flex flex-col gap-2">
            <div className="bg-black/80 border border-zinc-800/50 backdrop-blur-xl p-3 rounded-xl flex items-center gap-2"><Terminal size={14} className="text-purple-400 animate-pulse" /><span className="text-[10px] font-mono tracking-wider font-bold text-zinc-400 uppercase">Technology Division</span></div>
            {Object.keys(featureDetails).map((key) => {
              const IconComponent = featureDetails[key].icon;
              return (
                <button key={key} onClick={() => setActiveFeature(key)} className={`w-full flex items-center justify-between p-3 rounded-xl border font-mono text-xs tracking-wider font-bold text-left transition-all cursor-pointer ${activeFeature === key ? "bg-purple-600/20 border-purple-500 text-white shadow-lg" : "bg-black/75 border-zinc-800/60 text-zinc-400 hover:text-zinc-200"}`}>
                  <div className="flex items-center gap-3"><IconComponent size={16} className={featureDetails[key].color} /><span className="uppercase">{key}</span></div>
          ...
          <div className="md:col-span-5">
            {activeFeature && (
              <div className="bg-black/85 border border-zinc-800/80 backdrop-blur-2xl p-6 rounded-2xl h-full flex flex-col justify-center animate-fade-in shadow-2xl">
                ...
                <p className="text-zinc-300 text-xs font-sans leading-relaxed">{featureDetails[activeFeature].desc}</p>
              </div>
            )}
          </div>
        </div>
